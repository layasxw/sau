...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadMotivationData() async {
    ...
    try {
      ...
      final symptoms = results[0];
      final meals = results[1];
      ...
      if (todaySymptoms.isNotEmpty) {
        ...
        if (allSeverities.isNotEmpty) {
          final avg =
              allSeverities.reduce((a, b) => a + b) / allSeverities.length;
          symptomsHigh = avg > 3;
        }
      }
          ...
          Row(
            ...
            children: [
              Column(
                ...
                children: [
                  Text(
                      '${Translations.get(lang, 'hello')} ${_fullName?.split(' ')[0] ?? ''}!',
                      style: Theme.of(context).textTheme.displayLarge),
                  const SizedBox(height: 4),
                  Text(_getTodayLabel(lang),
  ...
  Widget _buildQuickActionsTitle(AppLanguage lang) => Padding(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(Translations.get(lang, 'quick_actions'),
                style: Theme.of(context).textTheme.headlineMedium),
            TextButton(
                onPressed: () {},
                child: Text(Translations.get(lang, 'view_all'),
                    style: const TextStyle(
